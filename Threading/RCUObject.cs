...
namespace SystemEx.Threading {
	...
	public class RCUObject<T> : IEquatable<RCUObject<T>>, ILockedObject<T> {
		...
		public Result ReadValue (int timeout = -1 ) {
			using (var _le = new ScopedLock<ILock>(ref m_writerLock, timeout) ) {
				var _res = new Result();

				_res[VALUE_INDEX] = m_object;
				_res[STATE_INDEX] = m_writerLock.IsLocked ? RCUState.Update : RCUState.Current;
