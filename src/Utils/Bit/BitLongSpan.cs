...
namespace SystemEx.Utils {
	...
	/// <summary>
	/// Provides a low-level bit window over a referenced <see cref="long"/> value.
	...
	public ref struct BitLongSpan {
        public ref struct Enumerator : IEnumerator<bool> {
            ...
            /// </summary>
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            internal Enumerator ( BitLongSpan span ) {
                m_span = span;
                m_index = -1;
            }
		...
		/// <summary>
		/// Gets the total number of bits in a <see cref="long"/> value.
		/// </summary>
		public short Bits => (short)(sizeof(long) * 8);
		...
		/// <summary>
		/// Initializes a new <see cref="BitLongSpan"/> over the specified referenced
		...
		public BitLongSpan ( ref long value, short startPos, short endPos, FlexSpanMode mode ) {
            m_value = ref value;
            m_mode = mode;
            m_lEnd = endPos;
            m_lStart = startPos;
		...
		/// </summary>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BitLongSpan Slice ( short start, FlexSpanMode? mode = null )
            => new BitLongSpan(ref m_value, (short)(m_lStart + start), (short)(m_lEnd - start), mode ?? m_mode);
		...
		/// </summary>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
        public BitLongSpan Slice ( short start, short length, FlexSpanMode? mode = null )
            => new BitLongSpan(ref m_value, (short)(m_lStart + start), length > Bits ? Bits : length, mode ?? m_mode);

		/// <summary>
		/// Determines whether two bit spans reference the same underlying integer
		/// and expose the same window and indexing mode.
		/// </summary>
		public static bool operator == ( BitLongSpan left, BitLongSpan right ) =>
